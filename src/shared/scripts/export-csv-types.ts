...
function generateSchemas() {
  ...
  csvFiles.forEach((file) => {
    ...
    const outputPath = path.join(outputDir, "csv.ts");
...
function createInterface(name: string, fields: string[]): string {
  ...
  const props = fields.map((f) => `  ${quoteField(f)}: string;`).join("\n");
  return `${comment}\n\nexport interface ${name}CSVSchema {\n${props}\n}\n`;
}
...
// Gunakan ini sebagai pengganti sanitizeField
function quoteField(field: string): string {
  // Jika field cocok dengan identifier TS biasa, langsung pakai
  if (/^[a-zA-Z_$][a-zA-Z0-9_$]*$/.test(field)) {
    return field;
  }
  // Jika tidak valid (misal mengandung - atau spasi), gunakan ['field-name']
  return `['${field}']`;
}
